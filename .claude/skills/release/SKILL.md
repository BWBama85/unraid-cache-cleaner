...
> **✅ The checkpoint is enforced by tracked `permissions.ask` rules (#51).** `.claude/settings.json` carries `Bash(git push:*)`, `Bash(git tag -a:*)`, and `Bash(gh release create:*)` under `permissions.ask`. Claude Code evaluates permissions **deny → ask → allow**, and `ask`/`deny` rules apply in **every** mode (including `bypassPermissions`) and override any `allow` regardless of scope — so each of the three writes prompts no matter which scope granted it. This closes the two silent traps that made absence-from-the-allow-list an unreliable checkpoint:
...
>
> The `ask` rules override both. `tests/test_release_permissions.py` guards them: it fails if a rule is dropped or a release write is reworded out from under its rule. (Tightening this skill's `allowed-tools` off bare `Bash` is optional defense-in-depth — the `ask` rules alone are sufficient — tracked separately.) `Bash(git push:*)` is repository-wide, so pushes from other flows (`/implement-issue`, `/resolve-pr-threads`) also prompt once; that breadth is deliberate — a narrower release-only pattern could be dodged by rewording the push.
