...
> **✅ The checkpoint is enforced by tracked `permissions.ask` rules (#51).** `.claude/settings.json` carries `permissions.ask` rules for `git push`, `gh release create`, and every annotated-`git tag` spelling that creates the same tag (`-a`/`--annotate`, `-m`/`--message` — both imply `-a` — `-am`, and the signed `-s`/`--sign` forms), so a reworded tag command cannot dodge the prompt while the read-only `git tag --list`/`-l`/`--sort` queries stay unprompted. Claude Code evaluates permissions **deny → ask → allow**, and `ask`/`deny` rules apply in **every** mode (including `bypassPermissions`) and override any `allow` regardless of scope — so each write prompts no matter which scope granted it. This closes the two silent traps that made absence-from-the-allow-list an unreliable checkpoint:
> - **This skill's `allowed-tools: Bash`** grants *bare* `Bash`, which on its own would otherwise pre-approve every command it runs — including `git push` / `git tag` / `gh release create`.
> - **A broad local `settings.local.json` grant** (e.g. `Bash(gh release *)` or `Bash(git push:*)`) would otherwise re-permit the writes on that one machine.
>
> The `ask` rules override both **when they are loaded**. `tests/test_release_permissions.py` guards them: it fails if a rule is dropped or a release write is reworded out from under its rule. `Bash(git push:*)` is repository-wide, so pushes from other flows (`/implement-issue`, `/resolve-pr-threads`) also prompt once; that breadth is deliberate — a narrower release-only pattern could be dodged by rewording the push.
>
> **⚠️ Residual gap — launch from the repository root.** Claude Code loads `settings.json` keys only from the **cwd's** `.claude/` folder, with *no* parent-directory fallback ([permissions docs](https://code.claude.com/docs/en/permissions#settings-discovery)), whereas skills *are* discovered up the tree. So launching `claude` from a **subdirectory** would still find this skill (and its bare `allowed-tools: Bash`) but would **not** load these project-root `ask` rules — re-opening the bypass. Project-scoped `ask` rules therefore fully cover only the normal root-launch case. Closing the subdir case needs one of: tightening this skill's `allowed-tools` off bare `Bash` (**#102** — not merely optional given this gap), or replicating the three `ask` rules in user-level `~/.claude/settings.json` / managed settings (which *do* load regardless of cwd). Until then: **run `/release` from the repo root.**
