...
| `WEB_ACTION_INLINE_SCRIPT` | `false` | Opt into the self-contained inline enhancement scripts (each admitted by a fresh per-response `script-src 'nonce-…'`, no external asset). Off keeps the strict no-script CSP. When `true` (and actions are on): (1) the reclaim form gains select-all + a live selected-count/size total (no `fetch`/XHR); (2) the in-flight **report-regeneration** status page live-polls `GET /api/rescan` and reflects completion without the no-JS full-page meta-refresh — this one page (and only it) also gains a scoped `connect-src 'self'` so the poll may `fetch`; a persistent poll failure (15 in a row — ~1 min when they fail fast, ~2 min for hung polls, which are aborted at a 4s deadline and counted the same; e.g. the unlock session lapsing mid-scan) navigates to the status/locked page rather than retrying forever, showing a fixed "still checking…" hint while it degrades. Both degrade gracefully: with JavaScript disabled the form still submits and the status page still meta-refreshes |
...
- **Non-blocking.** Report generation fans out to Plex/`*arr` and can take a minute or
  ...
  touches Plex). `GET` routes therefore still never reach Plex. With the inline-script
  enhancement on, the live-poll rides out a transient blip but stops after 15 consecutive
  failures — e.g. if the unlock session lapses mid-scan — and navigates to the status page
  (which then renders the locked page) instead of spinning indefinitely. That is ~1 min
  when polls fail fast; a poll that opens a connection and then *hangs* is aborted at its
  4s deadline and counted the same way, so sustained hangs take ~2 min to reach the same
  terminal page. While it is failing but still retrying, the page appends a fixed
  "still checking…" hint, cleared as soon as a poll succeeds.
