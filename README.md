...
- **Non-blocking.** Report generation fans out to Plex/`*arr` and can take a minute or
  ...
  enhancement on, the live-poll rides out a transient blip but stops after ~1 min of
  solid failure — e.g. if the unlock session lapses mid-scan — and navigates to the
  status page (which then renders the locked page) instead of spinning indefinitely.
